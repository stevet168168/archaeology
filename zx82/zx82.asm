...
N0081:  INC     HL
        LD      A,(HL)
        CP      $20             ; test if space
        JR      Z,N0081         ; back to SKIP-OVER-1 to try next byte if so
...
;; LINE-ADDR
L196E:  LD      B,H             ; move line number to BC register
        LD      C,L
...
; --------------------------
; This routine allows a one-byte instruction to stack one of 32 constants
; held in a table. In fact only 5 constants are required.
; On entry the A register holds the offset into the table.
